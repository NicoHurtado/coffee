...
export const useSettingsStore = create<State>()((set, get) => ({
  ...
  setLastUsedAccount: async (lastUsedAccountId) => {
    const previous = get().lastUsedAccountId
    ...
    void fetch("/api/settings", {
      ...
      .catch(() => {
        // Do not overwrite a newer account selection made while saving.
        if (get().lastUsedAccountId === lastUsedAccountId) {
          set({ lastUsedAccountId: previous })
        }
      })
  },
}))
