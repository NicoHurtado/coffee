...
export function ExpensesBreakdown() {
  ...
  return (
    <section className="surface rounded-2xl p-4 md:p-5 space-y-3 xl:flex-1">
      ...
      {total === 0 ? (
        ...
        <>
          <TooltipProvider delayDuration={80}>
            <div className="flex h-3 w-full gap-0.5 rounded-full overflow-hidden bg-muted">
              {totals.map((c) => {
              ...
              return (
                <div key={c.category} className="flex items-center justify-between gap-2 text-sm min-w-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="truncate text-[12px] tracking-[-0.005em]">{c.category}</span>
                  </div>
